...
interface NavbarProps {
  currentView: 'landing' | 'booking';
  setView: (view: 'landing' | 'booking') => void;
  onNavigateToSection: (sectionId: string) => void;
}

export default function Navbar({ currentView, setView, onNavigateToSection }: NavbarProps) {
          ...
          >
            ...
            <div>
              <span className="text-lg font-display font-extrabold tracking-tight text-pitch-charcoal flex items-center gap-1">
                RUNMAKERS<span className="text-emerald-700 text-[10px] border border-emerald-300/40 px-1.5 py-0.5 rounded font-sans font-bold bg-emerald-50">ARENA</span>
              </span>
            </div>
          </div>
        ...
        <div className="md:hidden bg-white/95 backdrop-blur-lg border-b border-slate-200 shadow-lg">
          <div className="px-4 pt-2 pb-6 space-y-3 sm:px-3">
            <button
              ...
              className="block w-full text-left px-3 py-2 rounded-lg text-sm font-semibold text-pitch-slate-500 hover:text-pitch-charcoal hover:bg-slate-50"
            >
              Home
            </button>
            <button
