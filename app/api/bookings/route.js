...
export async function POST(request) {
  try {
    ...
    if (sanitizedName.length < 2 || sanitizedName.length > 50) {
      return NextResponse.json({ error: 'Customer Name must be between 2 and 50 characters.' }, { status: 400 });
    }
    ...
    if (!phoneRegex.test(cleanedPhone)) {
      return NextResponse.json({ error: 'Invalid Indian mobile number. Must be a valid 10-digit number.' }, { status: 400 });
    }
    ...
    if (!utrRegex.test(cleanedUTR)) {
      return NextResponse.json({ error: 'Invalid UTR format. UPI transaction ID must be exactly 12 digits.' }, { status: 400 });
    }

    // 5. Validate Slot IDs are integers
    if (!slotIds.every(id => Number.isInteger(Number(id)))) {
      return NextResponse.json({ error: 'Invalid slot IDs format.' }, { status: 400 });
    }
    ...
    if (utrCheck.rows.length > 0) {
      ...
      return NextResponse.json({ error: 'This transaction UTR has already been submitted for another booking.' }, { status: 400 });
    }
    ...
    if (slotCheck.rows.length !== slotIds.length) {
      await query('ROLLBACK');
      return NextResponse.json({ error: 'Some slots were not found in the database.' }, { status: 404 });
    }
    ...
    for (const slot of slotCheck.rows) {
      if (slot.is_booked) {
        await query('ROLLBACK');
        return NextResponse.json({ error: `Slot starting at ${slot.start_time} is already booked.` }, { status: 400 });
      }
      ...
      const slotDateTime = new Date(year, month, day, parseInt(hours), parseInt(minutes), 0);

      if (slotDateTime < now) {
        await query('ROLLBACK');
        return NextResponse.json({ error: `Cannot book slots in the past (Slot: ${slot.start_time} on ${d.toLocaleDateString()}).` }, { status: 400 });
      }
    }
