...
/**
 * Parses slot date and time into a JavaScript Date object in Asia/Kolkata (IST).
 * @param {string} dateStr - YYYY-MM-DD date string
 * @param {string} timeStr - HH:MM start time string
 * @returns {Date}
 */
function getSlotDateTime(dateStr, timeStr) {
  if (!dateStr || !timeStr) return new Date(0);
  const yyyymmdd = dateStr.includes('T') ? dateStr.split('T')[0] : dateStr;
  const cleanTime = timeStr.slice(0, 5);
  return new Date(`${yyyymmdd}T${cleanTime}:00+05:30`);
}

/**
 * Checks if a slot is cancellable (at least 6 hours in the future).
 * @param {string} dateStr - YYYY-MM-DD date string
 * @param {string} timeStr - HH:MM start time string
 * @returns {boolean}
 */
function isCancellable(dateStr, timeStr) {
...
// Timezone-standardized date and time utilities exported for general application usage
module.exports = {
  getIstDateString,
  getIstTodayString,
