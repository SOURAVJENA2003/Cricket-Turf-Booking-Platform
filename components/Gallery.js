    ...
    <section id="arena-gallery" className="bg-transparent border-b border-slate-200/40 py-16 md:py-24 relative overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <AnimatePresence>
          {selectedItem && (
            <div 
              className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-pitch-charcoal/80 backdrop-blur-sm animate-fade-in"
            ...
            >
              <motion.div 
                ...
                className="bg-white rounded-3xl max-w-3xl w-full overflow-hidden border border-slate-200 shadow-2xl relative"
                onClick={(e) => e.stopPropagation()}
              >
                <button 
                  onClick={() => setSelectedItem(null)}
                  className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-black/60 hover:bg-black/85 text-white flex items-center justify-center transition-all cursor-pointer"
                  title="Close Modal"
                >
                  <X className="w-5 h-5" />
                </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12">
                  ...
                  <div className="md:col-span-5 p-6 flex flex-col justify-between text-left">
                    <div>
                      <span className="text-[9px] font-black uppercase tracking-wider text-emerald-700 bg-emerald-50 border border-emerald-150 px-2 py-0.5 rounded-full inline-block mb-3">
                        {selectedItem.category === 'action' ? 'Match Action' : selectedItem.category === 'turf' ? 'Infields' : 'Equipment'}
                      </span>
                      <h3 className="text-xl font-display font-black text-pitch-charcoal leading-tight">
                        {selectedItem.title}
                      </h3>
                      <p className="text-xs text-pitch-slate-500 font-sans mt-4 leading-relaxed">
                        {selectedItem.description}
                      </p>
                    </div>

                    <div className="pt-6 mt-6 border-t border-slate-100 flex items-center gap-2 text-[10px] text-pitch-slate-400 font-bold uppercase tracking-wider">
